...
async function removeAll(Model, data) {
    return new Promise(async (resolve, reject) => {
        try {
...
async function findDetails(Model, query, projection = {}, paginationParams = {}, populateOptions = []) {
    return new Promise(async (resolve, reject) => {
        try {
            const { skip, limit, sort, search, searchFields } = paginationParams;
            ...
            let dataQuery = Model.find(mongoQuery, projection);

            if (Array.isArray(populateOptions) && populateOptions.length > 0) {
                populateOptions.forEach((p) => dataQuery.populate(p));
            }

            if (typeof skip === 'number' && !isNaN(skip) && typeof limit === 'number' && !isNaN(limit) && limit > 0) {
...
// read with populate
async function readWithPopulate(Model, query, projection = {}, populateOptions = []) {
    return new Promise(async (resolve, reject) => {
        try {
            let dataQuery = Model.findOne(query, projection);
